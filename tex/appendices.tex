...
\begin{itemize}
   \item  Suppose by some magic black box (Concorde!) one has obtained the TSP tour through $n$ points.  
          Can we then compute the Delaunay Triangulation of the point-set in $O(n)$ time? It certainly seems so going by the high number of edges 
          in the TSP common to the Delaunay as suggested by the tests in this writeup. Maybe the Okabe book on Voronoi etc. has something on this?
          It's, of course, complete baloney to compute triangulations like this in practise but when did \textit{that} ever stop a computer scientist 
          from being metaphysical? \smiley
   ...
   \item  Is the Min Weight (MW), or even better, the Min-Angle Maximizing (MAM) Triangulation of the TSP and its external pockets close 
          ...
          Note that both MW and MAM triangulations of simple polygons can be computed by D.P. 
\end{itemize}  
