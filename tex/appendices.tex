...
As has already been mentioned, Python has excellent YAML data parsers, and so I've converted the data in the TSPLIB files 
into YAML format. 
     ...
     \begin{itemize}
      ...
      \item For $G=MST^3$, how good is the any (or the shortest??) Hamilton cycle through $G$
       ...
       \item Given a set of points in $\RR^2$, does *ANY* 
             ...
             given a set of points $P$ in general position, might there exist a triangulation of $P$ that shares at least one edge with  
             \textit{every} simple polygon on $P$? Maybe Hugo's counter-example of \autoref{fig:hugo-example} still holds? Qian's beautiful UGP 
                           counterexample may also be of some relevance.
      \end{itemize}
...
\begin{itemize}
   \item  Suppose by some magic black box (Concorde! \Winkey) one has obtained the TSP tour through $n$ points.  
          ...
          It's, of course, utter baloney trying to compute good triangulations or MSTs in this manner but 
          why should  that \textit{that} stop a computer scientist speculating from the comfort of his armchair?
          ...
          from a \textit{given} MST in linear time. His overall paper hinged on a very simple and nice generalization of the triangle inequality 
