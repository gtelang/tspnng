...
For computing the $k$-NNG we compute from $k=1$ upto a value of $k=2+\sqrt{n}$ where $n$ is the number of input points.
This allows a ``natural'' way to scale the value of $k$ for increasing values of $n$. 
...
for k in range(1,2+int(np.sqrt(len(run.points)))): 
